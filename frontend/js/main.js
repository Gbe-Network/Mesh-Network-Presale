...
async function ensureWeb3(timeoutMs = 8000) {
  ...
  return new Promise((resolve) => {
    ...
    const onReady = () => {
      try {
        removeEventListener('solanaWeb3#ready', onReady);
        removeEventListener('libs-ready', onReady);
      } catch {}
      if (globalThis.solanaWeb3 && globalThis.solanaWeb3.Transaction) {
        web3Loaded = true;
        finish(true);
    ...
    addEventListener('solanaWeb3#ready', onReady, { once: true });
    addEventListener('libs-ready', onReady, { once: true });
